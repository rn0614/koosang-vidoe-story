...
function renderNode(
  ...
  const isRenderableMesh =
    (node as any).geometry && (node as any).material

  const children = node.children.map((child) =>
    renderNode(child, partColors, onPartClick, wheelRotation, onWheelClick, isDoorOpen, doorAngle, handleInteriorClick, interiorScale)
  )
  ...
  if (isRenderableMesh) {
    ...
    if (Array.isArray(mesh.material)) {
      ...
      material.forEach((mat: any) => {
        if (mat.color && partColors[node.name] && COLORABLE_PARTS.includes(node.name)) {
          mat.color.set(partColors[node.name])
        }
      })
    } else {
      material = mesh.material.clone()
      if ((material as any).color && partColors[node.name] && COLORABLE_PARTS.includes(node.name)) {
    ...
    // 바퀴 회전 적용 (Z축)
    const wheelEuler = isRotatable ? new THREE.Euler(0, 0, wheelRotation, 'XYZ') : undefined
    ...
    if (node.name === 'door_l' || node.name === 'door_r') {
      ...
      return (
        ...
        >
          ...
          />
          {/* 자식들 재귀 렌더링 */}
          {node.children.map(child => renderNode(child, partColors, onPartClick, wheelRotation, onWheelClick, isDoorOpen, doorAngle, handleInteriorClick, interiorScale))}
        </group>
      )
    }
...
const CarModel: React.FC<CarModelProps> = ({ partColors, selectedColor, onPartClick }) => {
  const gltf = useGLTF('/assets/free_car_001.glb')
  return (
    <group scale={1.5} position={[0, -0.5, 0]}>
      {renderNode(gltf.scene, partColors, onPartClick, 0, () => {}, false, 0, () => {})}
    </group>
  )
}
...
const DoorAnimator: React.FC<DoorAnimatorProps> = ({
  ...
  useFrame((_, delta) => {
    ...
    setDoorAngle((prev) => {
      ...
      const step = Math.sign(diff) * Math.min(Math.abs(diff), delta * DOOR_ANIMATION_SPEED)
...
const Car3DPage: React.FC = () => {
  ...
  // 카메라 위치 상태 추가
  const [cameraPosition, setCameraPosition] = useState(new THREE.Vector3(0, 1.5, 5))
  ...
  const handlePartClick = (partName: string) => {
    setPartColors((prev) => ({ ...prev, [partName]: COLORS[selectedColorIdx].value }))
  }
  ...
  if (isSkyboxMode) {
    return (
      <div style={{ width: '100vw', height: '100vh', background: '#000', position: 'relative' }}>
        ...
        <Canvas camera={{ position: [0, 0, 0.1], fov: 75 }} style={{ width: '100vw', height: '100vh' }}>
    ...
    <div style={{ position: 'relative', height: '100%', background: '#222' }}>
      ...
      <div style={{
        ...
        {COLORS.map((c, idx) => (
          <button
            ...
            style={{
              padding: '10px 20px',
              background: selectedColorIdx === idx ? c.value : '#fff',
              color: selectedColorIdx === idx ? '#fff' : '#222',
              border: selectedColorIdx === idx ? `2px solid ${c.value}` : '1px solid #ccc',
              borderRadius: 8,
              fontWeight: 'bold',
              cursor: 'pointer',
              boxShadow: selectedColorIdx === idx ? '0 2px 8px rgba(0,0,0,0.15)' : undefined,
              outline: 'none',
              transition: 'all 0.2s',
              minWidth: 60,
            }}
      ...
      <button
        ...
        style={{
          ...
          zIndex: 10,
          padding: '10px 20px',
          background: isDoorOpen ? '#fbbf24' : '#3b82f6',
          color: '#222',
          border: 'none',
          borderRadius: 8,
          fontWeight: 'bold',
        ...
        disabled={isDoorAnimating}
      >
        {isDoorOpen ? '문 닫기' : '문 열기'}
      </button>
      <Canvas camera={{ position: [cameraPosition.x, cameraPosition.y, cameraPosition.z], fov: 50 }} shadows>
        <CameraController 
