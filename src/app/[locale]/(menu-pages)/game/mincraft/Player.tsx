...
export function Player({ lerp = MathUtils.lerp }: PlayerProps) {
  const axe = useRef<Group>(null);
  const rigidBodyRef = useRef<any>(null);
  const rapier = useRapier();
  const [, get] = useKeyboardControls();
  const { isJumping, setIsJumping, setPosition, setVelocity } =
    usePlayerStore();
  const debugRayRef = useRef<Line>(null);
  const [jumpStartVelocity, setJumpStartVelocity] = useState<{
    x: number;
    z: number;
  } | null>(null);
  ...
  useFrame((state, delta) => {
    ...
    if (axeChild) {
      ...
      const targetRotation =
        Math.sin((speed > 1 ? 1 : 0) * state.clock.elapsedTime * 10) / 6;
    ...
    if (debugRayRef.current) {
      const rayStart = new Vector3(origin.x, origin.y - 0.5, origin.z);
      const rayEnd = new Vector3(
        origin.x,
        origin.y - 0.5 - GROUND_THRESHOLD,
        origin.z,
      );
      ...
      debugRayRef.current.geometry.setAttribute(
        'position',
    ...
    const grounded =
      ray &&
      ray.collider &&
      ray.timeOfImpact <= GROUND_THRESHOLD &&
    ...
    const hasMovementInput =
      frontVector.length() > 0 || sideVector.length() > 0;
    ...
    const isCollidingHorizontally = horizontal_collisions.some(
      (ray) => ray && ray.collider && ray.timeOfImpact < 0.8,
    );
