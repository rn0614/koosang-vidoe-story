...
export default function WorkflowCreatePage() {
  ...
  return (
    <div className="max-w-xl mx-auto p-8">
      <h2 className="text-2xl font-bold mb-4">템플릿에서 내 워크플로우 생성</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block mb-1 font-semibold">템플릿 선택</label>
          <select
      ...
      {/* 내 워크플로우 리스트 */}
      <div className="mt-10">
        <h3 className="text-lg font-bold mb-2">내 워크플로우</h3>
        {myWorkflows.length === 0 ? (
          <div className="text-gray-500">내 워크플로우가 없습니다.</div>
        ) : (
          <ul className="space-y-2">
            {myWorkflows.map(wf => (
              <li key={wf.id} className="border rounded px-3 py-2 bg-white hover:bg-gray-50 transition">
                <Link href={`/workflow/${wf.id}`} className="block">
                  <div className="font-semibold">{wf.name}</div>
                  <div className="text-xs text-gray-400">생성일: {wf.created_at}</div>
                </Link>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-10">
        <h3 className="text-lg font-bold mb-2">내가 승인자로 포함된 워크플로우</h3>
        {approvalWorkflows.length === 0 ? (
          <div className="text-gray-500">승인자로 포함된 워크플로우가 없습니다.</div>
        ) : (
          <ul className="space-y-2">
            {approvalWorkflows.map(wf => (
              <li key={wf.id} className="border rounded px-3 py-2 bg-white">
                <div className="font-semibold">{wf.name}</div>
                <div className="text-xs text-gray-400">생성일: {wf.created_at}</div>
              </li>
            ))}
