...
interface WorkflowContextType {
  ...
  subscribeToGlobalChanges: (callback: () => void) => () => void;
...
export const WorkflowProvider: React.FC<WorkflowProviderProps> = ({
  ...
  // 글로벌 구독/해제
  const subscribeToGlobalChanges = useCallback((callback: () => void) => {
  ...
  // 글로벌 구독자들에게 알림
  const notifyGlobalSubscribers = useCallback(() => {
  ...
  const contextValue = useMemo<WorkflowContextType>(() => ({
    ...
    subscribeToGlobalChanges, getNode, getRelatedNodes, getAllNodeIds, getConnections,
...
export const useAllNodeIds = () => {
  ...
  const { subscribeToGlobalChanges } = React.useContext(WorkflowContext)!;
  
  // 글로벌 변경사항 구독 (setInterval 대신)
  React.useEffect(() => {
    ...
    return unsubscribe;
  }, [getAllNodeIds, subscribeToGlobalChanges]);
